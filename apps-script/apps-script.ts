...
function doPost(e: AppsScriptEvent) {
  ...
  try {
    ...
    const incomingData: IngestedPayload = JSON.parse(e.postData.contents);
    ...
    const targetSheet = getTargetSheet(ss);

    // 3. Dynamic Self-Healing Schema Expansion
    let columnCount = targetSheet.getLastColumn();
    let schemaHeaders: string[] = [];

    if (columnCount > 0) {
      // Fetch existing headers from Row 1
      schemaHeaders = targetSheet.getRange(1, 1, 1, columnCount).getValues()[0].map(h => String(h).trim());
    }
    ...
    incomingData.rows.forEach((record) => {
      const rowData = schemaHeaders.map(header => record[header] !== undefined ? record[header] : "");
 ...
 */
function getTargetSheet(ss: GoogleAppsScript.Spreadsheet.Spreadsheet): GoogleAppsScript.Spreadsheet.Sheet {
  const targetName = "Gateloga_Database";
  let sheet = ss.getSheetByName(targetName);
  if (sheet) return sheet;

  const fallbackNames = ["Logs", "Logbook"];
 ...
 */
function saveToFallbackSheet(ss: GoogleAppsScript.Spreadsheet.Spreadsheet, rawPayload: string, errorMessage: string) {
  try {
    const backupName = "Gateloga_Failed_Payloads";
...
function writeAuditLog(
  ...
  try {
    const auditName = "Gateloga_Audit_Logs";
    let auditSheet = ss.getSheetByName(auditName);
    if (!auditSheet) {
